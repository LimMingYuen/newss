...
class WindowRefTuple {
    constructor(nm, reference, url = '', targetEnv = undefined) {
        this.name = '';
        this.url = '';
        this.isLoaded = false;
        this.isActive = false;
...
class WrlStateStore {
    constructor() {
        ...
        this.wrlServiceLocatorUri = computed(() => this._state().wrlServiceLocatorUri);
        this.initialized = computed(() => this._state().initialized);
        this.allowLocalResources = computed(() => this._state().allowLocalResources);
        this.onlyLocalResources = computed(() => this._state().onlyLocalResources);
        this.localWebResources = computed(() => this._state().localWebResources);
     ...
     */
    setLocalResources(localWebResources) {
        this._state.update(state => ({
            ...state,
...
class WrlLocationTransparentServiceUriResolverService {
     ...
     */
    async getLocationTransparentServiceEndpoint(wrlConfig) {
        ...
        try {
            ...
            if (serviceResponse?.services?.apl[wrlConfig.site] == null) {
                this.loggerService.warn(`getLocationTransparentServiceEndpoint:There is no WRL service endpoint in transperent configuration  ${locationTransparentEndpoint} for site '${wrlConfig.site}'.`, serviceResponse);
                return endpointUri;
            }
            ...
            if (siteApl?.version) {
                ...
                this.loggerService.warn(`getLocationTransparentServiceEndpoint:Invalid response for from uri ${locationTransparentEndpoint} for site '${wrlConfig.site}'.`, siteApl);
            }
        }
        catch (error) {
    ...
    getWrlLocationTransparentEndpoint(wrlConfig) {
...
class WrlWebResourcesResolverService {
    ...
    async getWebResourcesServiceEndpoint(wrlConfig) {
...
class WrlApplicationInstanceManagerService {
    ...
    async setWrlServiceEndointAsync() {
        ...
        try {
            const wrlConfig = { env: envType, site: site, wrlServiceLocatorUri: wrlServiceLocatorUri };
...
class WebResourceService {
     ...
     */
    async initWrl(wrlConfig, resourceSelectionStrategy = undefined, resourceNotFoundHandler = undefined) {
        ...
        if (!resourceNotFoundHandler) {
            this.resourceNotFoundHandler = (applicationName, resourceName) => {
                this.logger.error(`No web resource found for name: ${applicationName} and resource name: ${resourceName} in ${wrlConfig.site}|${wrlConfig.env}!`);
            };
        }
        else {
     ...
     */
    getResourceUrisFromWrlLocal(applicationName, resourceName) {
        ...
        if (foundLocalResources.length === 0) {
            this.logger.warn(`No local resources found for ${applicationName} and ${resourceName}`, this.stateStore.localWebResources());
        }
        return foundLocalResources;
    }
    async loadWrlLocalResources(wrlConfig) {
...
class UfeAuthComponent {
    constructor() {
        ...
        this.logout = output();
        this.languageToggle = output();
        this.logoutLabel = 'Logout';
        this.menuExternalLinks = computed(() => this._externalLinks()?.links ?? this.externalLinks()?.links ?? []);
        this.menuHelpLinks = computed(() => [...this._helpLinks(), ...(this.helpLinks() ?? [])]);
        this._externalLinks = signal(undefined);
        this._helpLinks = signal([]);
...
class UfeHeaderComponent {
    constructor() {
        ...
        this.hideJwtCounter = input(false);
        this.hideAuthComponent = input(false);
        this.logout = output();
